...
class App {
  ...
  _setMesh() {
    ...
    this.material = new THREE.MeshToonMaterial({ color: 0xf55568 });
  ...
  render() {
    ...
    this.array.forEach((box, index) => {
      if (index % 2) {
        box.position.x += Math.sin(time - index) / 90;
        box.position.y += Math.cos(time - index) / 90;
        box.position.z += Math.cos(time - index) / 90;

        box.scale.x += -Math.sin(time - index) / 100;
        box.scale.y += -Math.cos(time - index) / 100;
        box.scale.z += -Math.sin(time - index) / 100;
      }
    });
